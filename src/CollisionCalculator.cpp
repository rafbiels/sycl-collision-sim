...
constexpr sycl::float3 wallNormal(WallCollision collision) {
    return {(collision & WallCollision::Xmax) - (collision & WallCollision::Xmin),
            (collision & WallCollision::Ymax) - (collision & WallCollision::Ymin),
            (collision & WallCollision::Zmax) - (collision & WallCollision::Zmin)};
}
...
void collideWorldParallel(std::vector<Actor>& actors, const Magnum::Range3D& worldBoundaries, size_t numAllVertices) {
    ...
    */
    ...
    for (size_t iVertex{0}; iVertex<numAllVertices; ++iVertex) {
        ...
        // Corrade::Utility::Debug{} << "[GPU] vertex " << iVertex
        //                           << " collision type " << static_cast<uint8_t>(collisions[iVertex])
        //                           << ", results = ("
        //                           << addLinearVelocity[iVertex][0] << ", "
        //                           << addLinearVelocity[iVertex][1] << ", "
        //                           << addLinearVelocity[iVertex][2] << "), ()"
        //                           << addAngularVelocity[iVertex][0] << ", "
        //                           << addAngularVelocity[iVertex][1] << ", "
        //                           << addAngularVelocity[iVertex][2] << ")";
    }
